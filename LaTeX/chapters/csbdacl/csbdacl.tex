...
The \glsentryshort{hol} representation of \glsentrylongpl{wff} (WFFs) are shown in figure \ref{FormACL}.
...
Kripke structures are defined previously as a three-tuple.  The Kripke structure here has four components (each surrounded by parentheses). This representation in \glsentryshort{hol} is very different from that described in section \ref{kripkesemantics}. 
...
The key take away for the reader is to recognize Kripke structures in the \glsentryshort{hol} code.  Kripke structures will either be fully typed or abbreviated.  These two forms are shown below, respectively.  
\[ (M, Oi, Os) \]
...
It remains now to define the access-control logic (\glsentryshort{acl}) formulas in \glsentryshort{hol} specifically.  These are shown in figure \ref{aclformulasHOL}.
...
Triangular brackets enclose propositions.  Thus, in the \glsentryshort{hol} representation of the \glsentryshort{acl}, a proposition is codes as follows:
\[\texttt{prop command} \]
...
\caption{\label{satdef}The \glsentryshort{hol} implementation of the "satisfies" property.  Image from  \textit{Certified Security by Design Using Higher Order Logic}\cite{certmanual}}
\end{figure}
...
The definition for ``satisfies" in the \glsentryshort{hol} is similar to the definition for "soundness" described in section \ref{sssec:soundness}.  In that section and the section preceding it, "satisfies" applied to a particular Kripke structure.  But, definition for ``satisfies" above is valid for all Kripke structures.  To extend this to ``soundness" it is only necessary to account for multiple hypotheses.  This is of the form shown in figure \ref{soundhol}


\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{../figures/soundhol}
\caption{\label{soundhol}The \glsentryshort{hol} representation of the ``soundness" property.  Image from  \textit{Certified Security by Design Using Higher Order Logic}\cite{certmanual}}
\end{figure}

A thorough description of the \glsentryshort{acl} in its entirety is beyond the scope of this master thesis.  All the theory files necessary to use this \glsentryshort{hol} implementation of the \glsentryshort{acl} are included with the files with this master thesis.  A pretty-printed EmitTeX version of the code is included in appendix \ref{ppacl}.  
